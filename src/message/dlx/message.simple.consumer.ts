...
export class MessageSimpleConsumer {
    private readonly logger = new Logger(MessageSimpleConsumer.name);
    ...
    constructor(
        ...
        this.createConsumerConnection(url, exchange, keys, handler, DLXExchange);
    }
    ...
    private createConsumerConnection(
        ...
        const simpleHandler = async (data) => {
            // handler(data)
            //     .then(() => {
            ...
            //     });
            this.logger.error(`[AMQP-CM]: Consume error for message ${JSON.stringify(data)}`);
        ...
        this.channel = this.connection.createChannel({
            ...
            setup: channel => {
                return Promise.all([
                    ...
                    channel.prefetch(1),
                    channel.assertQueue(queue, {durable:true, deadLetterExchange: DLXExchange}),
                    keys.forEach(key => channel.bindQueue(queue, exchange, key)),
                    channel.consume(queue, simpleHandler)
                ]);
            },
        });
